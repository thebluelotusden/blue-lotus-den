...
const projects = {
    ...
    'submerged-shadows': {
        ...
        heroImage: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 400 300'><rect width='100%' height='100%' fill='%23e0e0e0'/></svg>",
        detailImages: [
            "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 300'><rect width='100%' height='100%' fill='%23e5e5e5'/></svg>",
            "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 300'><rect width='100%' height='100%' fill='%23dcdcdc'/></svg>",
            "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 200 300'><rect width='100%' height='100%' fill='%23d3d3d3'/></svg>",
...
function handleRoute() {
    ...
    if (hash === '#projects') {
        ...
        panelProjects.querySelector('.panel-content').scrollTop = 0;
    } else if (hash === '#information') {
        panelInfo.classList.add('active');
        navInfo.classList.add('active');
        // Scroll panel content to top when opened
        panelInfo.querySelector('.panel-content').scrollTop = 0;
        ...
        if (projects[projectId]) {
            ...
            panelDetail.querySelector('.panel-content').scrollTop = 0;
        } else {
...
/* ==========================================================================
   3. DYNAMIC CONTENT LOADING: PROJECT DETAIL
   ========================================================================= */
function loadProjectDetail(id) {
    const data = projects[id];
    
    // Populate simple nodes
    document.getElementById('detail-project-title').textContent = `${data.title}, ${data.year}`;
...
function initInteraction() {
    ...
    closeBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            // If we are in project detail, going back should take us to projects list
    ...
    topLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            const panelContent = link.closest('.panel-content');
            if (panelContent) {
